...
Todos los API requests incluyen el header `X-Response-Time`:

```http
X-Response-Time: 45ms
```

Requests lentas (> 1000ms) se logean automáticamente con nivel WARN.
...
Abre visualización interactiva de bundles en el navegador.
...
│   │   │   ├── products/       # Endpoints de inventario
...
│   │       ├── loans.repository.ts   # Acceso a datos (Supabase)
│   │       └── loans.calculator.ts   # Funciones puras de cálculo
│   ├── schemas/                # ⭐ Validación con Zod
│   │   ├── loan.schema.ts      # Schemas de préstamos
│   │   ├── client.schema.ts    # Schemas de clientes
│   │   ├── sale.schema.ts      # Schemas de ventas
│   │   └── index.ts            # Re-exports
│   ├── components/             # Componentes React
...
│   ├── FASE5_OPTIMIZATION.md  # Optimizaciones aplicadas
│   └── ...
